...
| ![Live](https://img.shields.io/badge/Live-009E57?style=flat)  | Adds `vector.lerp()` to the standard Luau library. |
...
| ![Live](https://img.shields.io/badge/Live-009E57?style=flat)  | In Luau's New Type Solver, fix some instances where refinements would cause function calls to report a false positive type error. |
| ![Live](https://img.shields.io/badge/Live-009E57?style=flat)  | Adjust script analysis warnings and type hover from Luau to refer to the top type of foreign function types (e.g. from the Roblox API) as `userdata`. |
...
| Status | Note |
|--------|------|
| ![Live](https://img.shields.io/badge/Live-009E57?style=flat)  | Fixes an issue where scroll bar transparency changes were ignored when the `Class.ScrollingFrame` was not interactable. |
| ![Live](https://img.shields.io/badge/Live-009E57?style=flat)  | Fixes a potential infinite loop when re-capturing focus on the same text box. |
...
| ![Live](https://img.shields.io/badge/Live-009E57?style=flat)  | In Luau's New Type Solver, fix some instances where autocomplete/script analysis would fail to ever appear, and Studio would hang on closing. |

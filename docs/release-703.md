...
| ![Live](https://img.shields.io/badge/Live-009E57?style=flat)  | Luau's New Type Solver: Indexing into the result of the `setmetatable` type function is less likely to infer to `any` |
| ![Live](https://img.shields.io/badge/Live-009E57?style=flat)  | Luau's New Type Solver: Fixes some instances of "Type inference failed to complete" when trying to infer the types of globals captured by multiple functions. |
...
| ![Live](https://img.shields.io/badge/Live-009E57?style=flat)  | Luau's New Type Solver: Adjusts generic instantiation to never select `never` or `unknown`, ensuring user always gets a useful type. |

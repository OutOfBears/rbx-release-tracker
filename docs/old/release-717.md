...
| Status | Note |
|--------|------|
| ![Live](https://img.shields.io/badge/Live-009E57?style=flat)  | Fix blurry or smudged text when using TextScaled. |
| ![Live](https://img.shields.io/badge/Live-009E57?style=flat)  | A `Class.GuiObject` influenced by a `Class.UIScale` no longer has its `Class.GuiObject.Size\|Size` property uneditable in the **Properties** window. |
| ![Live](https://img.shields.io/badge/Live-009E57?style=flat)  | Significant broadphase optimizations for geometry-dense maps, especially when large objects are moving (such as during dragging operations in Studio). |
| ![Live](https://img.shields.io/badge/Live-009E57?style=flat)  | Respect the `transparency` parameter for `Class.EditableImage` operations with the `combineType` parameter set to `Enum.ImageCombineType.Add`. |

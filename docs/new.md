...
| ![Pending](https://img.shields.io/badge/Pending-DEA517?style=flat)  | Fixes two bugs in `CompressorSoundEffect` and `EqualizerSoundEffect`, respectively, which could cause bursts of loud noise on Windows. |
| ![Pending](https://img.shields.io/badge/Pending-DEA517?style=flat)  | Fixes an bug where using the move/rotate/scale tools to reposition an `Attachment` could also reposition its parent `Model`, resulting in "doubled" movement. |

...
| Status | Note |
|--------|------|
| ![Live](https://img.shields.io/badge/Live-009E57?style=flat)  | Introduces `AnimationClip.Length` to allow easy access to the duration of an AnimationClip instance. |
...
| ![Live](https://img.shields.io/badge/Live-009E57?style=flat)  | Fixes a bug where opening/closing the Animation Editor would remove the `Class.Animator` on the rig and create a new one, thereby deleting all of its previous state. |
...
| ![Pending](https://img.shields.io/badge/Pending-DEA517?style=flat)  | Adds `Class.EditableImage` `antiAliased` parameter to `DrawCircle` and `DrawLine` to support drawing hard edges. |
